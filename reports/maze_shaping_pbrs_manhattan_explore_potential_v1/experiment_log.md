# Frontmatter
- Log file: `reports/maze_shaping_pbrs_manhattan_explore_potential_v1/experiment_log.md`
- Script: `experiments/maze_shaping_icml_style/run_maze_shaping_experiment.py`
- Output roots:
  - `outputs/maze_shaping_icml_style_pbrs_manhattan_explore_potential_v1`

# Goal
- 탐색 보상도 potential-based 형태로 추가한 실험.
...
- Final training (last episode):
  - `no_shaping`: mean_steps=`299.4166666666667` std=`113.10942956073801` (episode `500`)
...
- Final validation (last checkpoint):
  - `no_shaping`: success=`0.16666666666666666` val_steps=`299.0` (episode `500`)
  - `phi_full`: success=`0.0` val_steps=`350.0` (episode `500`)
  - `phi_half`: success=`0.0` val_steps=`350.0` (episode `500`)
- Best validation success:
  - `no_shaping`: success=`0.16666666666666666` val_steps=`299.0` (episode `350`)
