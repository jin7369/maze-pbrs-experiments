...
def run_train_episode(
    ...
    while True:
        ...
        is_revisit = (s2 in visited) and (not done_goal)
        if is_revisit:
            revisit_count += 1
        done_revisit = (revisit_count >= revisit_terminate_count) and (not done_goal)
        done = done_goal or done_revisit

        shaped_r = base_r + gamma * phi[s2] - phi[s] + (revisit_penalty if is_revisit else 0.0)
...
def run_eval_episode(env: MazeEnv, q: np.ndarray, revisit_terminate_count: int):
    s = env.reset()
    visited = {s}
    revisit_count = 0
    steps = 0
...
def train_and_validate(
    ...
    for ep in range(episodes):
        steps, success = run_train_episode(
            env,
            q,
            phi,
            alpha,
        ...
        if (ep + 1) % validation_interval == 0 or ep == 0:
            ...
            for _ in range(validation_episodes):
                s_steps, s_ok = run_eval_episode(env, q, revisit_terminate_count)
                succ += int(s_ok)
                total_steps += s_steps
            val_records.append(
                {
    ...
    for scale in scales:
        ...
        for run_idx in range(runs):
            q, steps, val_df = train_and_validate(
                ...
                revisit_terminate_count=revisit_terminate_count,
...
def plot_learning_curve(df: pd.DataFrame, out_path: Path):
    ...
    plt.title("Manhattan PBRS with revisit-penalty threshold termination")
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
...
def plot_validation(df: pd.DataFrame, out_path: Path):
    ...
    axes[1].set_title("Validation steps to termination (threshold)")
...
def main():
    parser = argparse.ArgumentParser(description="Manhattan PBRS experiment with revisit-penalty and threshold termination.")
    parser.add_argument("--maze-path", type=Path, default=Path("신규_실험_workspace/outputs/maze_samples_v1/grids/maze_00.npy"))
    parser.add_argument("--outdir", type=Path, default=Path("신규_실험_workspace/outputs/maze_shaping_revisit_penalty_threshold_manhattan_v1"))
    ...
    parser.add_argument("--revisit-penalty", type=float, default=-0.05)
    parser.add_argument("--revisit-terminate-count", type=int, default=3)
    ...
    run_experiment(
        ...
        revisit_terminate_count=args.revisit_terminate_count,
        seed=args.seed,
    )
